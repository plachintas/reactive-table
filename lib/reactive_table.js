...
var generateSettings =  function () {
    ...
    Session.setDefault(getSessionFilterKey(group), settings.initialFilterValue || null);
...
Template.reactiveTable.rendered = function () {
    ...
    Deps.autorun(function(){
        var group = tpl.$('.reactive-table-filter .reactive-table-input').attr('reactive-table-group');
        var clearBtn = tpl.$('.reactive-table-filter .reactive-table-input-clear');

        var filterText = Session.get(getSessionFilterKey(group));
        clearBtn.toggle(!!filterText);
    });
};
...
Template.reactiveTable.events({
    ...
    'click .reactive-table-input-clear': function (event) {
        var clearBtn = $(event.target);
        var group = $(event.target).parents('.reactive-table-filter').attr('reactive-table-group');
        Session.set(getSessionFilterKey(group), null);
        Session.set(getSessionCurrentPageKey(this.group), 0);
    },
